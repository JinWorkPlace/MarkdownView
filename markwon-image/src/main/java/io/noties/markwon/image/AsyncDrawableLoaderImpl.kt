...
internal class AsyncDrawableLoaderImpl @VisibleForTesting constructor(
    builder: AsyncDrawableLoaderBuilder, private val handler: Handler
) : AsyncDrawableLoader() {
    ...
    constructor(builder: AsyncDrawableLoaderBuilder) : this(
        builder, Handler(Looper.getMainLooper())
    )
    ...
    private fun execute(asyncDrawable: AsyncDrawable): Future<*>? {
        return executorService?.submit {
            ...
            try {
                ...
                check(!(scheme == null || scheme.isEmpty())) { "No scheme is found: $destination" }
                ...
                if (schemeHandler != null) {
                    ...
                    if (imageItem.hasDecodingNeeded()) {
                        val withDecodingNeeded = imageItem.getAsWithDecodingNeeded()
                        ...
                        try {
                            ...
                            drawable = mediaDecoder?.decode(
                                withDecodingNeeded.contentType(), withDecodingNeeded.inputStream()
                            )
                        } finally {
                            try {
