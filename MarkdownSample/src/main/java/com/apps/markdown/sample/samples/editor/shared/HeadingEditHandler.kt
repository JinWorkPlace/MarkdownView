...
class HeadingEditHandler : EditHandler<HeadingSpan> {
    ...
    override fun configurePersistedSpans(builder: PersistedSpans.Builder) {
        builder.persistSpan<Head1>(Head1::class.java) { Head1(theme!!) }
            .persistSpan<Head2>(Head2::class.java) { Head2(theme!!) }
    }
    ...
    override fun handleMarkdownSpan(
        ...
        if (type != null) {
            ...
            val end = if (index < 0) input.length
            else index
            editable.setSpan(
                persistedSpans.get(type), spanStart, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
            )
        }
    }
