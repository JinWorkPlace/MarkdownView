...
"""
    ...
    private fun fetch() {
        ...
        cancellable = sampleManager.samples(sampleSearch) {
            val addVersion =
                sampleSearch is SampleSearch.All && TextUtils.isEmpty(sampleSearch.text)
    ...
    companion object {
        ...
        fun markwon(context: Context): Markwon {
            return Markwon.builder(context).usePlugin(MovementMethodPlugin.none()).build()
        }
    ...
    private data class State(
        val search: String?, val recyclerScrollPosition: RecyclerScrollPosition?
    ) : Parcelable

    @Parcelize
    private data class RecyclerScrollPosition(
        val position: Int, val offset: Int
    ) : Parcelable
