...
import io.noties.markwon.image.DrawableUtils.intrinsicBounds

open class AsyncDrawable(
    @JvmField val destination: String, private val loader: AsyncDrawableLoader,
    /**
     * @since 4.0.0
     */
    ...
    // @since 4.5.0
    private val placeholder: Drawable? = loader.placeholder(this)
    ...
    fun hasKnownDimensions(): Boolean {
        return this.lastKnownCanvasWidth > 0
    }
    ...
    fun setCallback2(cb: Callback?) {
        ...
        this.callback = if (cb == null) null else WrappedCallback(cb)
        ...
        if (callback != null) {
            ...
            if (result != null) {
                ...
                if (result is Animatable) {
                    wasPlayingBefore = (result as Animatable).isRunning
                    val isPlaying = wasPlayingBefore
                    if (isPlaying) {
                        (result as Animatable).stop()
                    }
                }
            }
    ...
    protected fun setPlaceholderResult(placeholder: Drawable) {
        ...
        if (rect.isEmpty) {
            ...
            val intrinsic = intrinsicBounds(placeholder)
    ...
    private fun initBounds() {
        if (this.lastKnownCanvasWidth == 0) {
    ...
    override fun toString(): String {
        return "AsyncDrawable{" + "destination='" + destination + '\'' + ", imageSize=" + imageSize + ", result=" + result + ", canvasWidth=" + this.lastKnownCanvasWidth + ", textSize=" + this.lastKnowTextSize + ", waitingForDimensions=" + waitingForDimensions + '}'
    }

    // @since 4.2.1
    //  Wrapped callback to trigger invalidation for this AsyncDrawable instance (and not result/placeholder)
    private inner class WrappedCallback(private val callback: Callback) : Callback {
    ...
    companion object {
        ...
        private fun noDimensionsBounds(result: Drawable?): Rect {
            if (result != null) {
                ...
                val intrinsicBounds = intrinsicBounds(result)
