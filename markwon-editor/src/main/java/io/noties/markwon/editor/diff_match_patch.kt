 ...
 */
internal object diff_match_patch {
    /**
     ...
     */
    @JvmStatic
    fun diff_main(text1: String, text2: String): LinkedList<Diff> {
     ...
     * @return Linked List of Diff objects.
     */
    private fun diff_main(text1: String, text2: String, checklines: Boolean): LinkedList<Diff> {
        // Check for null inputs.
        var text1 = text1
        var text2 = text2
        require(!(text1 == null || text2 == null)) { "Null inputs. (diff_main)" }
    ...
    private fun diff_compute(
        text1: String, text2: String, checklines: Boolean
    ): LinkedList<Diff> {
        ...
        if (hm != null) {
            ...
            val text2_a = hm[2]
            val text2_b = hm[3]
            val mid_common: String? = hm[4]
            // Send both pairs off for separate processing.
            val diffs_a = diff_main(
                text1_a, text2_a, checklines
            )
            val diffs_b = diff_main(
                text1_b, text2_b, checklines
            )
            // Merge the results.
            diffs = diffs_a
     ...
     * @return Linked List of Diff objects.
     */
    private fun diff_lineMode(text1: String, text2: String): LinkedList<Diff> {
        ...
        val pointer: MutableListIterator<Diff> = diffs.listIterator()
     ...
     */
    private fun diff_bisect(text1: String, text2: String): LinkedList<Diff> {
...
//            }
    ...
    private fun diff_bisectSplit(
        text1: String, text2: String, x: Int, y: Int
    ): LinkedList<Diff> {
        ...
        val diffs = diff_main(text1a, text2a, false)
        val diffsb = diff_main(text1b, text2b, false)
    ...
    private fun diff_charsToLines(
        diffs: MutableList<Diff>, lineArray: MutableList<String?>
    ) {
     ...
     */
    private fun diff_commonSuffix(text1: String, text2: String): Int {
        // Performance analysis: https://neil.fraser.name/news/2007/10/09/
        val text1_length = text1.length
        val text2_length = text2.length
    ...
    private fun diff_commonOverlap(text1: String, text2: String): Int {
        ...
        val text_length = min(text1_length, text2_length)
...
//        }
     ...
     */
    private fun diff_cleanupSemantic(diffs: LinkedList<Diff>) {
        if (diffs.isEmpty()) {
            return
        }
        var changes = false
        val equalities: Deque<Diff> = ArrayDeque<Diff>() // Double-ended queue of qualities.
    ...
    private val BLANKLINEEND: Pattern = Pattern.compile("\\n\\r?\\n\\Z", Pattern.DOTALL)
    private val BLANKLINESTART: Pattern = Pattern.compile("\\A\\r?\\n\\r?\\n", Pattern.DOTALL)
    ...
    private fun diff_cleanupMerge(diffs: LinkedList<Diff>) {
        ...
        if (diffs.lastOrNull()?.text?.isEmpty() == true) {
            diffs.removeLast() // Remove the dummy entry at the end.
        }
        ...
        while (pointer.hasNext()) {
            if (prevDiff!!.operation == Operation.EQUAL && nextDiff?.operation == Operation.EQUAL) {
                ...
                if (thisDiff.text!!.endsWith(prevDiff.text!!)) {
                    ...
                    thisDiff.text =
                        (thisDiff.text!!.substring(nextDiff.text!!.length) + nextDiff.text)
                    pointer.remove() // Delete nextDiff.
                    nextDiff = if (pointer.hasNext()) pointer.next() else null
                    changes = true
                }
    ...
    class LinesToCharsResult(
        var chars1: String, var chars2: String, var lineArray: MutableList<String?>
    )
